          ...
          <div className={`rounded-3xl p-4 sm:p-8 shadow-xl ${currentTeam === 'COCA' ? 'bg-red-600 text-white' : currentTeam.startsWith('FWCH') ? 'bg-[#0d1b4d] text-white' : currentTeam.startsWith('FWCI') ? 'bg-gradient-to-r from-green-100 via-blue-100 to-red-100' : 'bg-white'}`}>
            ...
            <div className="grid lg:grid-cols-2 overflow-hidden rounded-[2rem] border-4 border-slate-200 bg-white">
              {currentTeam.startsWith('FWCH') ? (
                <div className="lg:col-span-2 grid lg:grid-cols-2 bg-[#0d1b4d]">
                  {[historyPageItems[currentTeam].slice(0, historyPageItems[currentTeam].length / 2), historyPageItems[currentTeam].slice(historyPageItems[currentTeam].length / 2)].map((pageItems, pageIndex) => (
                    <div key={`${currentTeam}-page-${pageIndex}`} className={`p-3 sm:p-8 ${pageIndex === 0 ? 'border-b lg:border-b-0 lg:border-r border-slate-300' : ''} bg-[#0d1b4d]`}>
                      <div className="grid grid-cols-2 gap-2 sm:gap-4">
                        {pageIndex === 0 && (
                          <div className="col-span-2 mb-1 sm:mb-2">
                            <div className="text-xl sm:text-3xl font-black uppercase leading-none text-slate-100">
                              FIFA WORLD CUP HISTORY
                            </div>
                            <div className="mt-2 text-base sm:text-2xl font-black uppercase text-slate-200">
                              ⭐ WORLD CHAMPIONS
                            </div>
                          </div>
                        )}
                        ...
                        {pageItems.map((item, index) => {
                          if (item.type === 'printed') {
                            return (
                              <div
                                key={`${currentTeam}-printed-${pageIndex}-${index}`}
                                className="border-2 border-slate-500 rounded-xl sm:rounded-2xl p-2 sm:p-4 w-full flex items-center justify-center text-center aspect-[3/2] bg-slate-700 text-slate-200"
                              >
                                <div className="italic uppercase text-[10px] sm:text-sm mt-1 leading-tight font-black">
                                  IMPRESO<br />{item.label}
                                </div>
                              </div>
                            );
                          }
